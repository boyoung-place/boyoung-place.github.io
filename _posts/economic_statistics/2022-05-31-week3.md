...
- **결합분포(joint distribution)**: 두 변수 간의 관계 전모를 보여줌

  - ex. 교육과 임금, 통화증가율과 물가상승률, 또는 학급 규모와 학생 성적
  - 여기에서 상관계수의 개념이 나오게 된다.

- **산포도(scatter plot)**: 데이터가 얼마나 그리고 어떻게 퍼져있나를 나타냄

  - 두 변수 사이의 관계를 살펴보기 위해 산포도를 이용한다.
  - 설명변수는 x로 표기하고 가로축에 표시하며, 피설명변수는 y로 표기하고 세로축에 표시한다.

  ![scatter_plot](https://bokyeong-kim.github.io/assets/img/testscore.png)
  ...
  - 위의 산포도 그림에서 x와 y는 양(+)의 관계이다.
    ...
    - 산포도의 요약
      ...
      ![scatter_plot_summary](https://bokyeong-kim.github.io/assets/img/scatter.png)
...
- 상관계수의 범위, 부호
  ...
  - 평균점을 원점으로 두고 구간을 나누어 보았을 때, 사분면(quadrant)이 확인된다.
